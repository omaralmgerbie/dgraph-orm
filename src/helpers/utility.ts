...
export const checkOptions = (
  ...
  Object.keys(options).forEach((key: string) => {
    ...
    if (typemap[options.type].indexOf(key) === -1) {
      console.log(
        `${options.type} ${key} ${typemap[options.type].indexOf(key)}`,
      );

      throw new Error(
        "[Type Error]: In " + name + " of type " + options.type.toUpperCase() +
          ", " + key + " is not allowed.",
      );
    }
  });
  ...
  if (
    !options.token && options.index &&
    (options.type === types.DATETIME || options.type === types.STRING)
  ) {
    throw new Error(
      "[Type Error]: In " + name + " of type " + options.type.toUpperCase() +
  ...
  if (
    options.type === types.DATETIME && options.index &&
  ...
  if (
    options.type === types.DATETIME && options.index &&
    typeof options.token === "string" &&
    tokenmap[types.DATETIME].indexOf(options.token) === -1
  ) {
    throw new Error(
      "[Type Error]: In " + name + " of type " + options.type.toUpperCase() +
        ", token must be any one of " + tokenmap[types.DATETIME].join(", ") +
        ".",
    );
  }
  ...
  if (
    ...
    Object.keys(options.token).forEach((key: string) => {
      if (tokenmap[options.type].indexOf(key) === -1) {
        throw new Error(
          "[Type Error]: In " + name + " of type " +
            options.type.toUpperCase() + ", for token only " +
            tokenmap[options.type].join(", ") + " are allowd.",
        );
      }
    });
  }
...
export const prepareSchema = (
  ...
  Object.keys(options).forEach((key: string) => {
    if (
      key === "type" || key === "list" || key === "index" || key === "token" ||
