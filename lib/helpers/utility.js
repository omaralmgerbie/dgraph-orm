...
exports.merge = exports.pluck = exports.prepareSchema = exports.checkOptions = void 0;
...
var checkOptions = function (name, options) {
    Object.keys(options).forEach(function (key) {
        ...
        if (typemap_1.default[options.type].indexOf(key) === -1) {
            console.log(options.type + " " + key + " " + typemap_1.default[options.type].indexOf(key));
            throw new Error("[Type Error]: In " + name + " of type " + options.type.toUpperCase() +
                ", " + key + " is not allowed.");
        }
    });
    ...
    if (!options.token && options.index &&
        (options.type === types_1.default.DATETIME || options.type === types_1.default.STRING)) {
        throw new Error("[Type Error]: In " + name + " of type " + options.type.toUpperCase() +
            ", index requires token.");
    }
    if (options.type === types_1.default.DATETIME && options.index &&
    ...
    if (options.type === types_1.default.DATETIME && options.index &&
        typeof options.token === "string" &&
        tokenmap_1.default[types_1.default.DATETIME].indexOf(options.token) === -1) {
        throw new Error("[Type Error]: In " + name + " of type " + options.type.toUpperCase() +
            ", token must be any one of " + tokenmap_1.default[types_1.default.DATETIME].join(", ") +
            ".");
    }
    if (options.token && typeof options.token !== "string" &&
        options.type !== types_1.default.DATETIME) {
        ...
        Object.keys(options.token).forEach(function (key) {
            if (tokenmap_1.default[options.type].indexOf(key) === -1) {
                throw new Error("[Type Error]: In " + name + " of type " +
                    options.type.toUpperCase() + ", for token only " +
                    tokenmap_1.default[options.type].join(", ") + " are allowd.");
            }
        });
    }
};
